...
class MainQuestTracker
	...
	def setMainQuestStage(newStage)
		...
		if !getMainQuestStages.has_key?(newStage)
			pbMessage(_INTL("\\wmA recoverable error has occured: {1} is an invalid Main Quest Stage key. The \"What Next?\" will be inaccurate until the next Main Quest update. Please let a programmer know you saw this error, and where.", newStage))
			return
		end
